...
from app.database import fetch_one, health_database, is_postgres
...
def runtime_check() -> dict:
    ...
    ok = bool(db.get("ok")) and bool(storage.get("ok")) and bool(llm.get("configured")) and all(t.get("ok") for t in tables)
    return {
        "ok": ok,
        "database": db,
        "storage": storage,
