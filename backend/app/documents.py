...
def _insert_processed_document(mapped_user, mapped_notebook: str, upload_file: UploadFile, content: bytes):
    ...
    _log(f"Nome original={original_name!r} nome_storage={safe_name!r}")
    ...
    storage_path = upload_to_supabase_storage(target, f"users/{mapped_user}/notebooks/{mapped_notebook}/{unique_name}")
    ...
    if is_postgres():
        doc_id = execute(
            ...
            (user_id, notebook_id, filename, original_filename, file_type, file_size,
             storage_bucket, storage_path, local_path, status, character_count, chunk_count)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                mapped_user, mapped_notebook, safe_name, original_name, suffix.replace('.', ''), file_size,
