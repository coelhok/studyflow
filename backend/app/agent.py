...
OUT_OF_SCOPE_HINTS = [
    "corinthians", "palmeiras", "flamengo", "noticia", "notícias", "hoje", "tempo agora", "cotação", "preço", "piada",
...
def get_documents(notebook_id, selected_document_ids: Iterable | None = None, user_id: str | int = "1") -> list[dict]:
    ...
    if selected:
        ...
        if docs:
            return docs
        _log(f"IDs selecionados não existem mais, não pertencem ao usuário ou não estão processados: {selected}. Usando fallback controlado do notebook.")

    # Sem seleção explícita ou com seleção obsoleta: usa até 3 documentos processados mais recentes do usuário.
    return _recent_documents(notebook_id, user_id, limit=3)
...
11. Para questionário, gere exatamente 5 questões por padrão, com alternativas A, B, C, D, gabarito e explicação curta.
...
def _task_instruction(tasks: list[str], mode: str) -> str:
    ...
    for task in tasks:
        if task == "summary":
            ...
            lines.append("- QUESTIONÁRIO: crie exatamente 5 questões objetivas com alternativas A, B, C, D, gabarito e explicação curta, usando somente os trechos.")
        elif task == "flowchart":
...
def _fallback_quiz(sentences: list[str]) -> str:
    base = sentences[:5]
    if not base:
        return "Não encontrei base suficiente para criar questões com segurança."
    lines = []
    for i, sentence in enumerate(base, start=1):
        keyword = "conceito apresentado no documento"
        words = [w for w in re.findall(r"[A-Za-zÀ-ÿ]{5,}", sentence) if normalize(w) not in {"sobre", "entre", "tambem", "também", "documento"}]
        if words:
            keyword = words[0]
        lines.append(
            f"{i}. Segundo os trechos recuperados, qual alternativa melhor se relaciona a **{keyword}**?\n"
            f"   A) {sentence[:130]}{'...' if len(sentence) > 130 else ''}\n"
            f"   B) Uma afirmação sem relação com o documento.\n"
            f"   C) Uma conclusão que não aparece nos trechos.\n"
            f"   D) Um dado externo não citado nas fontes.\n"
            f"   **Gabarito:** A. A alternativa A é a única apoiada pelo trecho recuperado."
        )
    return "\n\n".join(lines)
...
def build_agent_answer(
    ...
    if plan.out_of_scope_risk and plan.tasks == ["free_question"] and not extract_query_terms(message):
